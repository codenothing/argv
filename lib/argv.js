var PATH = require( 'path' ),
	...
	helpOption = {
		...
		example: "'" + SCRIPT_NAME + " #MODULE_NAME# -h' or '" + SCRIPT_NAME + " #MODULE_NAME# --help'",
		onset: function( args ) {
			self.help( args.mod );
			process.exit( 0 );
		}
...
module.exports = self = {
	...
	mod: function( object ) {
		...
		//Add helpOption as default
		if(!object.options) {
			object.options = [];
		} 
		object.options.push(helpOption);
	...
	help: function( mod ) {
		...
		if ( mod && ( mod = self.mods[ mod ] ) ) {
			...
			for ( name in mod.options ) {
				...
				if ( option.example ) {
					output.push( "\t\t" + option.example.replace(MODULE_NAME, mod.mod) );
				}
		...
		// Printing out just the root options
			...
			for ( name in self.options ) {
				...
				output.push( "\t\t" + option.description );
				if ( option.example ) {
					output.push( "\t\t" + option.example.replace(MODULE_NAME, ""));
				}
